...
@pytest.mark.parametrize('mode', ['normal', '404', 'wrong_content', '500', 'business_error', 'exhausted',
                                 'delete_failed', 'extra_item', 'aborted', 'changed_before_delete', 'late_verify', 'late_cleanup', 'missing_once', 'missing_always', 'missing_first',
                                 'missing_collision', 'missing_late', 'missing_late_before'])
def test_failed_current_image_is_removed_before_retry(browser, tmp_path, mode, caplog, monkeypatch):
    ...
    if(MODE!=='missing_first')add('saved.png');
    window.selections=0;
    document.querySelector('input').onchange=async()=>{
      if(document.querySelectorAll('li').length!==(MODE==='missing_first'?0:1))window.overlapped=true;
      window.selections++;
      const r=await fetch('/permit/web/permit/savelicepic',{method:'POST',body:new FormData()});
      const body=await r.json();
      if(MODE.startsWith('missing')&&(window.selections===1||MODE==='missing_always'))return;
      if(MODE==='missing_late')add('old.png');
      add(body.data);
        ...
        if path.endswith('/savelicepic'):
            ...
            route.fulfill(status=500 if mode == '500' and bad else 200,
                          json={'status': 1 if mode in ('500', 'business_error') and bad else 0,
                                'data': ('saved.png' if mode == 'missing_collision' else 'old.png' if len(calls) == 1 else 'new.png') if mode.startswith('missing') else 'bad.png' if bad else 'new.png'})
        elif path == '/file':
    ...
    receipts = [] if mode == 'missing_first' else [saved]
    def trigger():
    ...
    def cleanup(responses, guard):
        missing = view._remove_failed_upload(card, receipts, responses, tmp_path/'new.png', '资质1', guard)
        if mode == 'missing_late_before':
            page.evaluate("add('old.png')")
        return missing
    def run():
        return upload_with_retry(page, trigger, lambda:card().locator('li').count(),
            ...
            verify_success=verify, cleanup_failed=cleanup)
    if mode in ('normal', '404', 'wrong_content', '500', 'business_error', 'missing_once', 'missing_first'):
        run()
        assert len(calls) == (1 if mode == 'normal' else 2)
        assert [x['id'] for x in preview_files(card())] == ([r.server_id for r in receipts] + ['new.png'])
    else:
        with pytest.raises(PageFlowError): run()
        assert len(calls) == (4 if mode == 'exhausted' else 2 if mode in ('late_verify', 'late_cleanup', 'missing_always', 'missing_late') else 1)
    assert page.evaluate('window.overlapped') is False
    assert 'saved.png' not in page.evaluate('window.deleted')
    if mode in ('changed_before_delete', 'late_verify', 'late_cleanup') or mode.startswith('missing'):
        assert page.evaluate('window.deleted') == []
    verify_preview_content(page, receipts, preview_files(card())[:len(receipts)], 1200)
    if mode == 'exhausted':
    ...
    if mode.startswith('missing') and mode != 'missing_collision':
        assert caplog.text.count('额外补传一次') <= 1
    page.close()
