...
10. 文件按顺序逐张上传，核对服务端文件标识、预览加载和保存后的内容哈希。只有请求明确返回可重试的服务端错误且未新增文件时，才最多重试 3 次；上传已成功但预览缺失、请求未结束、状态不明或部分成功时停止，避免重复上传。
...
推送版本标签（例如 `v0.30.10`）后，工作流会在所有平台构建成功后自动创建 GitHub Release 并附加四个安装包。
