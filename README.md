...
10. 上传异常时核对当前卡片已上传文件；确认未新增且请求已结束才重试，首次失败后最多重试 3 次。已存在的文件不重复上传；请求未结束、状态不明或部分上传成功时停止检查，避免重复。
...
推送版本标签（例如 `v0.30.9`）后，工作流会在所有平台构建成功后自动创建 GitHub Release 并附加四个安装包。
