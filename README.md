...
当前有效资质流程为新版 `/newaudit`“业务资质”。旧版 `/flice`“行业资质”逻辑已废弃，旧代码仅作历史保留；此次标记未改变运行分支。
...
10. 文件按顺序逐张上传，核对服务端文件标识、预览加载和保存后的内容哈希。新版每张文件还检查预览地址HTTP状态和内容哈希。请求明确结束且当前失败项可唯一定位时，先点击该项删除、确认其他成功文件完整，再最多重试3次（合计4次）；重试耗尽会清理当前失败项并停止当前公司。上传成功但没有新增条目时，确认原有文件完整且请求结束后，允许额外补传一次；仍缺失则停止，补传计入总重试上限。请求未结束、状态不明、存在条目但无法唯一定位或删除失败时停止，不盲目重传。
...
推送版本标签（例如 `v0.30.17`）后，工作流会在所有平台构建成功后自动创建 GitHub Release 并附加四个安装包。
