...
5. 每次任务点击“选择输入目录”，选择包含公司目录的根目录。
   在任务中心选择浏览器类型；Chrome/Edge 留空路径时自动查找，也可点击“选择程序”指定路径（Windows 选 `.exe`，macOS 选 `.app`）。其他 Chromium 浏览器必须指定程序。设置会自动保存，“清除路径”可恢复自动查找；切换类型会清除旧路径。Firefox 和 Safari 暂不支持。
...
推送版本标签（例如 `v0.30.8`）后，工作流会在所有平台构建成功后自动创建 GitHub Release 并附加四个安装包。
